...
def render_game():
        ...
        if st.button(btn_label, type="primary", use_container_width=True):
            if st.session_state.is_night:
                ...
                if st.session_state.reporter_target_day:
                    ...
                    msg_list.append(f"📰 [특종] 플레이어 {t_pid}님의 직업은 [{t_role}]로 밝혀졌습니다!")
    ...
    for i, (pid, pinfo) in enumerate(st.session_state.players_info.items()):
        ...
        with cols[i % 2]:
            is_disabled = (not pinfo['alive']) and (st.session_state.current_night_role != "의사")
